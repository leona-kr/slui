...
(function($){
	...
	$.Dashboard = {
		...
		url : {
			...
			report_list : '/dashboard/dashboard_report.do',
		...
		dashboardInsert : function(){
			var d = this,
			$btnAdd = $('.'+d.classname.groupTab+' .'+d.classname.btnAdd),
			$layer = $('.'+d.classname.modalAdd),
			_autoCloseLayer = function(event){
			...
			_eventInsert = function(){
				...
				$layer.find('input[type=text]').on('keyup',function(){
					if(event.keyCode === 13){
		...
		dashboardDelete : function(id, callback){
			...
			if(isOk){
				_confirm(d.msg.deleteDashboard,{
					onAgree : function(){
						$('body').requestData(d.url.dashboard_delete,{dashboard_id:id},{callback:function(){
							...
							$('.'+d.classname.groupTab+' li[data-id='+id+']').remove().promise().done(function(){
								...
								if(!hasActive) $('.'+d.classname.groupTab+' li:eq(0) .'+d.classname.btnTab).click();
							});
						}});
					}
				});
		...
		dashboardlist : function(_data, _newname){
			...
			_init = function(){
				...
				//전체보기 탭 토글 이벤트
				$listcontent.parent().siblings('.'+d.classname.btns).find("."+d.classname.btnToggle).off().on("mousedown",_openAll);
			},
			_loadContent = function(dashboard_id, isUpdate){
				//화면 및 데이터 초기화
				if( $('.window.dashboardsetting').is(':visible')){
			...
			_eventTab = function(){
				...
				$listcontent.find('.'+d.classname.btnLink).off().on('mouseenter',function(){
				...
				.end()
				.find('.'+d.classname.btnDelete).off().on('mousedown',function(){
			...
			_eventAll = function(){
				...
				$('.'+d.classname.groupTabs+' li').each(function(){
					...
					.end()
					.find('.'+d.classname.btnEdit).off().on('click',function(){
		...
		dashboardlistArrow : function(){
			...
			eventArrows = function(){
				if(innerSize > outerSize){
					...
					$btn_p.off().on('mousedown',move);
					$btn_n.off().on('mousedown',move);
				} else {
		...
		dashboardContent : function(_arg){
			...
			_init = function(){
				...
				$content
					.html('<div class="'+d.classname.grid+'" id="gridarea'+contentid+'"></div><div id="boardarea'+contentid+'" class="nano-content" style="right:-17px;"></div>')
					.addClass("nano")
					.css({
						'position' : 'fixed',
						'top' : po_t+'px',
				...
				$gridarea = $('#gridarea'+contentid);
				$contentarea = $('#boardarea'+contentid);
			...
			_drawGrid = function(){
				var grid_col = d.option.grid_col,
				 $grid = $('#gridarea'+contentid).show().empty(),
			...
			_eventHandle = function(){
				...
				// remove component
				$('#boardarea'+contentid).find('.'+d.classname.componentClose).off().on('mouseup',function(){
					var _this = $(this),
			...
			_setComponentlist = function(componentData){
				...
				for(var i=0;i<len;i++){
					if(_data[i].component_type==='') continue;
					var $li = $('<li />')
						.appendTo($target),
					$text = $('<span />')
						.addClass('txt-item')
						.text(_data[i].component_title)
						.appendTo($li),
					$btn = $('<button />')
						.attr('type','button')
						.attr('data-add-component','true')
						.data(_data[i])
						.addClass('btn-basic btn-apply')
						.text('추가')
						.appendTo($li)
				...
				//컴포넌트 윈도우
				$(".btn-addboard").off().on('click',function(){
					var _window = '.window.componentadd',
					_close = function(){
						$('.btn-addboard').removeClass(d.classname.open);
						$(_window).hide();
					},
					_open = function(){
						$('.btn-addboard').addClass(d.classname.open);
						$(_window)
							.show()
							.css({
						...
						$(_window+' .nano').nanoScroller();
						$(_window+' .modal-head').css('cursor','move');
						$(_window+' .btn-close').one('click',_close);
					};

					if($(this).hasClass(d.classname.open)){
						_close();
					} else {
						_open();
					}
			...
			_eventComponentadd = function(){
				...
				addComponent = function(data){
					...
					d.GRIDSTER.add_widget(html, min_unit, 10, 1, 1);
		...
		dashboardConfig : function(isrefresh){
			...
			//$window = $('.window.dashboardsetting'),
			_ = {
				name : '#setting_name',
				theme : '#setting_theme',
				grid : '#setting_grid',
			...
			_init = function(){
				//컴포넌트 설정창
				$(".group-side .btn-setting").off().on('click',function(){
				...
				// 리포트 다운로드
				$(".group-side .btn-report").off().on("click",function(){
			...
			_event = function(){
				...
				$(_.grid+' [data-switch-toggle=true]').off().on('click',function(){
					if($(this).val()==='0'){
				...
				//테마 선택
				$(_.theme+' .btn-theme').on('click',function(){
					var val = $(_.theme+' [name=dashboard_theme]:checked').parents('[class*=range-]').index();
					var theme_callback = function(data){
						$('.'+d.classname.groupTab+' .'+d.classname.active+' .'+d.classname.btnLink).click();
					}
					_update(val + 100, theme_callback);
				});

				//공유 변경
				$(_.share+' [data-switch-toggle=true]').off().on('click',function(){
					if($(this).val()==='0'){
