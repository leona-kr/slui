...
_SL.nmspc("logsearch").manager = function(){
	...
	urlVisualTookit		= gCONTEXT_PATH + "monitoring/va_toolkit.do?dummy",
	...
	init = function(serverTime, param) {
		...
		$("#btnSettingMyLogs").togglePage(gCONTEXT_PATH + "monitoring/mylogs_group_form.html");
...
slapp.logsearch.query = function() {
	...
	addKeyword = function(key, value) { //로그 검색결과를 클릭해서 keyword를 입력할때
		if(gIsMyLogsMode) {
		...
		if(regExpTest || regExpTest2 || sPreText == "") {
			//1.(OR|AND|NOT)검색연산자가 keyword의 마지막일때 2.+로 끝날때 3.처음 추가할때 4.keyword를 전체선택한 후(드래그 or 세번클릭) 새 키워드 추가할때는 그냥삽입
			str = key + ":" + _SL.luceneValueEscape(value);
		...
		//마우스 선택영역을 제외한 부분의 keyword에 같은 key가 있고 그 key가 선택영역의 앞쪽 keyword에 있을경우
			str = "OR " + key + ":" + _SL.luceneValueEscape(value);
		}else{//그 외에는 AND조건
			str = "AND " + key + ":" + _SL.luceneValueEscape(value);
		}
		
		replaceSelection(str, bLSpace, bRSpace);
		
		//$areaSearch.trigger('autosize.resize');
	},
...
slapp.logsearch.layer = function(){
	...
	init = function(){
		...
		slapp.logsearch.assets.init();	//_setAssetInit();
	...
	_eventMove = function(direction){
		...
		$(areaSearch).val( $li.eq(index).text().trim() );
	},
	_autosize = function(){
		// textarea 사이즈 자동 조절
		//var maxlength = 123;//1줄 표현 텍스트 길이 123);
	},
	_showLayer = function(){
...
slapp.logsearch.recent = function(){
	...
	setSave = function(){
		...
		if( q !='' ){
			...
			$.cookie(cname, rlist.toString(),{expires:30});
		}
	},
	setLists = function(){
		if(rlist.length > 0){
...
slapp.logsearch.assets = function(){
	...
	originListField = [], originListBook = [],

	assetcname = slapp.logsearch.option.assetName,

	searchField, searchBook,
	...
	init = function(mode){
		...
		if(mode == undefined){
			var selected = $.cookie(assetcname) != null ? $.cookie(assetcname) : '0';
			_setUi(selected);
		} else {
			_setUi(mode);
		}

		$(selectAsset).val(selected).on('change',function(){
	...
	_initCodebookValue = function() {
		...
		//$layerBookUl.append($wrapper);
	},
	_setUi = function(mode, isChange){
		if (mode === '0'){
			$(inputBook).hide();
			$(layerBook).hide();
			$(inputField).show().val('');
			...
			$(layerField+ ' ul').show();
			$(layerField).show();
			$(layerField).nanoScroller();

			if(isChange) searchField.init();

		} else if (mode ==='1'){
			$(inputField).hide();
			$(layerField).hide();
			$(inputBook).show().val('');
			$(layerBook+ ' .'+classListEmpty).hide();
			$(layerBook+ ' ul').show();
			$(layerBook).show();
			$(layerBook).nanoScroller();

			if(isChange) searchBook.init();
		}
	...
	_eventInit = function(){
		$(layerAsset+' .'+classList+' li:not(.'+classItemtitle+')').each(function(){
			...
			if( $(this).parents('.sp-list').attr('id') == 'layerField' ){
				originListField.push({
					'text' : $(this).text().toLowerCase()
				});
			}

			if( $(this).parents('.sp-list').attr('id') == 'layerBook' ){
				originListBook.push({
					'text' : $(this).text().toLowerCase()
				});
			}
...
slapp.logsearch.mysearch = function(){
	...
	mDlgMyfilterForm = function() {
		...
		init = function() {
			...
			//설정 창 만들기
			$dlg.jqxWindow({
				width: 640, height: 390, autoOpen: false,
				minWidth:640, minHeight: 390,
				maxWidth:640, maxHeight: 500,
				resizable: true, isModal: true, modalOpacity: 0.5,
				cancelButton : $(dlg+' .'+classCancel),
				initContent: function(){
			...
			_SL.setDataToForm(data, $form, {
				view_fields : {
					converter	: function(cvData, $fld) {
						if(cvData) {
							$("[name=view_fields]", $form).setSelectionOrder(cvData.split(","), true);
						}
					}
				},
				share_group_list : {
			...
			$('body').requestData(rqData.myfilter_id == "" ? urlInsert : urlUpdate, rqData, {
				callback : function(data, rsCd, rsMsg) {
					if(rqData.myfilter_id == "") {
						...
						$(listMysearch).find("[data-id="+data.myfilter_id+"]").text(data.myfilter_name);
...
ListSearchUi.prototype = {
	init : function(){
		var s = this;
		$(s.element).on('keyup', function(event){
