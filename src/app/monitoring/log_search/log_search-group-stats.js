...
_SL.nmspc("logsearch").statsDlg = function(){
	...
	mCfg = {
		...
		URL : {
			...
			timelineStats	: URL_PATH + "timeline_stats.html"
		}
	},
	...
	onClickBtnSearch = function () {
		if (m$.statsType.filter(":checked").val() == "group") {
			...
			var groupStatsModal = new ModalPopup(mCfg.URL.groupStats + "?" + $.param(params, true), {width:800, height:600, draggable:true});
		} else {
			
			if ($("#timeline_stats_func").val() != "count" && $("#timeline_func_field").val() == null) {
				alert("분석할 통계필드를 선택하세요.");
				return;
			}
			
			var seriesVals = [];
			if($("#timeline_series_field").val() !=""){
				$("[name=timeline_series_vals] option").attr("selected", true);
				if(!$("[name=timeline_series_vals]").val()){
					alert("범례 항목을 입력하세요");
					return;
				}else{
					seriesVals = $("[name=timeline_series_vals]").val();
				}
			}
			 
			var query = {
				start_time	: DynPaging.start_time,
				end_time	: DynPaging.end_time, 
				query		: DynPaging.query,
				period_min	: $("#period_min :selected").val(),
				is_complete : false,
				stats_func	: $("#timeline_stats_func").val(),
				func_field	: $("#timeline_func_field").val(),
				series_field: $("#timeline_series_field").val(),
				series_vals	: seriesVals,
				series_val_etc:$("input:radio[name='series_val_etc']:checked").val()
			};
			openWindow("time_stats_" + (new Date()).getTime() , "./time_stats.do?" + $.param(query,true) + "&series_vals"+seriesVals, 350, 1024, "yes", "yes");
		}
