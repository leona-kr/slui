					...
					success: function(resp, status, jqxhr){
						...
						//} else if (code === RESULT_CODE.SUCCESS){
							if(option.callback != null){
								var result =  resp.data;//var result =  $.extend({}, {result : 1}, resp.data);
 ...
 */
	...
	ExpandPage.prototype={
		...
		contentOverlay : function(isOpen){
			...
			if( isOpen ){
				$container.append('<div class="'+classLayerBlank+'" style="display:none;"></div>');
				$('.'+classLayerBlank).show().one('click',function(){
					$("[data-expand-button=true]").removeClass("open");
					that.collapsing(that);
				});
			} else {
 ...
 */
	var ModalPopup = function(url,options){
		...
		that.defaultOption = {
			...
			onOpen : null,
			onClose : null,			//저장후 닫을때 실행
			onDestroy : null,		//닫을때 실행
			onInit : null,
			type: 'get',
			data: null,
			displayLoader: false,
	...
	ModalPopup.prototype = {
		markup : function(){
			...
			_w = that.option.width,
			_h = that.option.height,
			_zindex = that.option.zIndex;
		...
		show : function($container){
			...
			if(that.option.effect == 'show'){
				...
				$container.fadeIn(that.option.speed);
			}
	...
	transformSelect.prototype = {
		...
		eventToggle : function($t,$p){
			...
			_event = function($ele){
				...
				$p.find('.'+_this.classOptionWrap).css('min-width', $t.width()+2+'px');
