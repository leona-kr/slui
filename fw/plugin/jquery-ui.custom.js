...
$.extend( Datepicker.prototype, {
	...
	_showDatepicker: function( input ) {
		...
		if ( !inst.inline ) {
			...
			/* insert seculayer 2016-07-12 */
			if( $(input).parents('[class^=page-]').size() >0){
				var _z = $(input).parents('[class^=page-]').css('z-index');
				if( _z === 'auto'){
					_z = $(input).parents('[class^=page-]').parents('[class^=page-]').css('z-index');
				}
				console.log(_z);
				inst.dpDiv.css( "z-index", _z+10 );
			} else {
