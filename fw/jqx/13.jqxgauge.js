    ...
    */
        ...
        _addTicks: function () {
            ...
            if (self.int64 === 's') {
                ...
                if (this.niceInterval) {
                    ...
                    for (var i = second, j = second; i.lessThan(this._max64) || j.lessThan(this._max64); i = i.add(minorStep), j = j.add(majorStep)) {
                        if (j.lessThanOrEqual(this._max64) && ticksMajor.visible) {
                            drawMajor(j);
                ...
                if (this.niceInterval) {
                    ...
                    for (var i = second, j = second; i.compare(this._max64) === -1 || j.compare(this._max64) === -1; i = i.add(minorStep), j = j.add(majorStep)) {
                        if (j.compare(this._max64) !== 1 && ticksMajor.visible) {
                            drawMajor(j);
                            oldVals[j.toString()] = true;
                ...
                if (this.niceInterval) {
                    ...
                    for (var i = second, j = second; i < this.max || j < this.max; i += minorStep, j += majorStep) {
    ...
    var common = {
        ...
        _getMaxLabelSize: function () {
            var maxVal = this.max,
                minVal = this.min;
            if (this._getLabelInterval().toString() < 1) {
        ...
        _getNiceInterval: function (type) {
            function log10(val) {
            ...
            if (that.labels.formatValue) {
                ...
                largestLabelSize = that._getMaxLabelSize()[dimension];
            }
    ...
    linearGauge = {
        ...
        _drawTicks: function (options, border, inactiveOffset) {
            ...
            function loop(i) {
                ...
                if (!options.checkOverlap || !self._overlapTick(i)) {
                    ...
                    if (self.niceInterval && self.labels.visible) {
                        ...
                        distance += border;
                        var maxLabelSize = self._getMaxLabelSize()[dimension],
                            maxLabelCounterDimension = self._getMaxLabelSize()[counterDimension],
                            side = options.isFar ? 'far' : 'near',
                            offset;
            ...
            if (self.niceInterval && options.type === 'major') {
                var second;
                if (self.int64 === 's') {
                    loop(self._min64);
        ...
        _overlapTick: function (value) {
            var majorInterval = this._getInterval('ticksMajor'),
                minorInterval = this._getInterval('ticksMinor');
        ...
        _renderLabel: function (distance, position, offset, maxLabelSize, currentValue) {
            ...
            formatedValue = this.labels.formatValue(currentValue.toString(), position);
        ...
        _valueToCoordinates: function (value) {
            ...
            return length + (this._width - offsetLeft - scaleLength) + border;
        },
