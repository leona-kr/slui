...
(function ($) {
    $.extend($.jqx._jqxGrid.prototype, {
        _initpager: function () {
            ...
            this.pagerdiv = this.pagerdiv || $('<div class="pagerdiv" style="width: 100%; height: 100%; position: relative;"></div>');
            if (!this.pageable) {
            ...
            if (!this.pagerrenderer) {
                ...
                this.pagergotoinput = this.pagergotoinput || $('<div style="margin-right: 7px; margin-left:4px; white-space:nowrap; height: 17px; float: right;"><input style="margin-top: 0px; text-align: right; width: 27px;" type="text"/>/<span style="margin-right:7px;"></span></div>');
                this.pagergoto = this.pagergoto || $('<div style="float: right; margin-top:2px;"></div>');
                this.pagerrightbutton = this.pagerrightbutton || $('<div type="button" style="padding: 0; margin-top:0; margin-right: 8px; width: 15px; float: right;"></div>');
                this.pagerleftbutton = this.pagerleftbutton || $('<div type="button" style="padding: 0px; margin-top:0; margin-right: 3px; margin-left:0; width: 15px; float: right;"></div>');
                ...
                this.pagerfirstbutton = $('<div type="button" style="padding: 0; margin-top: 0; margin-left: 3px; margin-right: 3px; width: 27px; float: right;"></div>');
                this.pagerlastbutton = $('<div type="button" style="padding: 0; margin-top: 0; margin-right: 3px; width: 27px; float: right;"></div>');
				this.totalrow = $('<div class="text-count">Total <b></b> rows</div>');
				this.iconexport = $('<div class="icons-export"></div>');

                this.pagershowrowscombo = this.pagershowrowscombo || $('<div id="gridpagerlist" style="margin-top: 0px; margin-left:7px; float: right;"></div>');
                ...
                if (this.pagermode == "simple") {
					...
					this.pagerdiv.append(this.totalrow);
					...
					this.pagerdiv.append(this.iconexport);
                }
                ...
                var leftarrow = $("<div style='width: 15px; height: 15px;'></div>");
                leftarrow.addClass(this.toThemeProperty('jqx-icon-arrow-left'));
                this.pagerleftbutton.wrapInner(leftarrow);

                var rightarrow = $("<div style='width: 15px; height: 15px;'></div>");
				...
				for(var i=0;i<source.length;i++){
					source_row.push( { 
						"index" : source[i],
						"text" : source[i]+" "+pagershowrowsstring
					} )
				};
                ...
                this.addHandler(this.pagershowrowscombo, 'select', function (event) {
                    if (event.args) {
                        ...
                        if (pagesize >= 100) {
                            me.pagershowrowscombo.jqxDropDownList({ width: me.widthPageSelect+7 });
                        }
                        else {
                            me.pagershowrowscombo.jqxDropDownList({ width: me.widthPageSelect });
                        }
        ...
        _updatepagertheme: function () {
            ...
            if (this.pagermode == "simple") {
                ...
                var lastarrow = $("<div style='width: 15px; height: 15px;'></div>");
                lastarrow.addClass(this.toThemeProperty('jqx-icon-arrow-last'));
                this.pagerlastbutton.wrapInner(lastarrow);
                this.pagerdiv.append(this.pagerfirstbutton);
                this.pagerdiv.append(this.pagerleftbutton);
		...
		_setexporticons: function(){
			if (this.pagermode !== 'simple') {
				var download = {};
				if(this.exportArea.length !=0){
					var icons = this.exportArea;
					for(var i=0, len =icons.length; i<len ;i++){
						var datatype = icons[i].exportFormat,
							 $button = $("<button />")
								.attr({
									"type" : "button",
									"title" : icons[i].text
								})
								.addClass("datatable-download");

						if ( datatype || datatype == "xls" || datatype == "html" || datatype == "pdf" || datatype == "json" || datatype == "xml") {
							$button.data("file-type",datatype)
						}else{
							$button.data("file-type","other")
								.data("onClick",icons[i].onClick);
						}
						if( icons[i].confirm == true){
							var msg = icons[i].confirmMsg ? icons[i].confirmMsg : this.confirmMsg;
							$button.data({"confirm": true, "confirmMsg" : msg});
						}

						$button.html('<span class="icon-file icon-'+icons[i].icon+'"></span>'+icons[i].text);

						download[i] = $button;
						this.iconexport.append(download[i]);
					}
				}
				var that = this
				this.iconexport.find("button").each(function(){
					var type = $(this).data("file-type");
					if( type == "other"){
						$(this).off().on("click",$(this).data("onClick"));
					} else {
						$(this).off().on("click",function(){
							if($(this).data("confirm") && $(this).data("confirmMsg")){
								if(!confirm($(this).data("confirmMsg"))) return false;
							}
							$("#"+that.element.id).jqxGrid('exportdata', type, 'jqxGrid');
						});
					}
				});
			}
        ...
        updatepagerdetails: function () {
            if (this.pagerdetails != null && this.pagerdetails.length > 0) {
				...
				var totalpage = Math.ceil(this.dataview.totalrecords/this.dataview.pagesize);
				this.pagergotoinput.find("span").html(totalpage);
				this.totalrow.find("b").html(this.dataview.totalrecords);
