...
(function ($) {
    ...
    $.extend($.jqx._jqxKnob.prototype, {
        ...
        propertyChangedHandler: function (object, key, oldvalue, value) {
            var that = this;
            if (key === 'value') {
                that._setValue(value, 'propertyChange');
                return;
            }
            that._validateProperties();
            that._refresh();
        ...
        _movePointers: function () {
            ...
            for (var i = 0; i< that._pointers.length; i++){
                if ( that._pointers.length !== 1) {
                    ...
                    that._pointers[i] = that._movePointer(that._pointers[i], that.pointer[i], angle);
                } else {
                    ...
                    that._pointers[0] = that._movePointer(that._pointers[0], that.pointer, angle);
                }
            }
        },

        _movePointer: function (pointerElement, pointerObject, angle) {
            var that = this;
