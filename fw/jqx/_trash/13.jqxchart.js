...
(function ($) {    
    ...
    jqxPlot.prototype = {
        ...
        {
            if (isNaN(val) || val < Math.min(range.min, range.max) || val > Math.max(range.min, range.max))
                return NaN;
            ...
            {            
                ...
                var maxPow = $.jqx.log(max, logBase);//$.jqx._rnd($.jqx.log(max, logBase), 1, max < 1 ? false : true);
                max = Math.pow(logBase, maxPow);

                var minPow = $.jqx.log(min, logBase);//$.jqx._rnd($.jqx.log(min, logBase), 1, false);
            ...
            {            
                ...
                var maxPow = $.jqx.log(scale_range.max, logBase);//$.jqx._rnd($.jqx.log(max, logBase), 1, max < 1 ? false : true);
                var minPow = $.jqx.log(scale_range.min, logBase);//$.jqx._rnd($.jqx.log(min, logBase), 1, false);
...
(function ($) {
    ...
    $.extend($.jqx._jqxChart.prototype,
    {
                ...
                if (group.polar || group.spider) {
                    ...
                    if (!isXAxis) {
                        ...
                        if (group.spider) {
                            ...
                            this._columnAsPieSlice(elementInfo, 0, rect, polarAxisCoords, columnRect);
        ...
        _calculateXOffsets: function (groupIndex, axisSize) {
            ...
            if (this.enableSampling && xoffsets.length > this.samplingThreshold)
                this._sampleXOffsets(axisSize, xoffsets, xvalues, first, last);
        ...
        _sampleXOffsets: function (axisSize, xoffsets, xvalues, first, last) {
            var last = NaN;
            for (var i = 0; i < xoffsets.length; i++) {
                var offset = Math.round(xoffsets[i]);
                if (isNaN(offset))
                    continue;

                if (!isNaN(last) && Math.abs(xoffsets[last] - offset) < 1)
                    continue;
    ...
    $.extend($.jqx._jqxChart.prototype,
    {
        ...
        */
        ...
        _refreshSelector: function (groupIndex, axis, axisStats, renderTo, rect, swapXY) {
            ...
            this.addHandler($(document), self._getEvent('mousemove') + '.' + this.element.id, self._onSliderMouseMove, { self: this, groupIndex: groupIndex, renderTo: renderTo, swapXY: swapXY });
            this.addHandler($(renderTo), self._getEvent('mousedown'), this._onSliderMouseDown, { self: this, groupIndex: groupIndex, renderTo: renderTo, swapXY: swapXY });
            this.addHandler($(document), self._getEvent('mouseup') + '.' + this.element.id, self._onSliderMouseUp, { element: this.element.id, self: this, groupIndex: groupIndex, renderTo: renderTo, swapXY: swapXY });
        },
        ...
        _createSliderElements: function (groupIndex, renderTo, rect, selectorSettings) {
            ...
            var div = $("<div class='slider' style='position: relative;'></div>");
        ...
        _onSliderMouseUp: function (event) {
            ...
            self._selectorRange[groupIndex] = { min: minValue, max: maxValue };
