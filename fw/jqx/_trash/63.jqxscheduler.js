        ...
        _updateTouchScrolling: function () {
            ...
            if (that.isTouchDevice()) {
                ...
                if (that.schedulercontent) {
                    that.removeHandler(that.schedulercontent, touchstart + '.touchScroll');
                    that.removeHandler(that.schedulercontent, touchmove + '.touchScroll');
                    that.removeHandler(that.schedulercontent, touchend + '.touchScroll');
                    that.removeHandler(that.schedulercontent, 'touchcancel.touchScroll');

                    $.jqx.mobile.touchScroll(that.schedulercontent[0], Math.max(that.vScrollInstance.max, that.hScrollInstance.max), function (left, top) {
...
//            that.refresh();
        ...
        this.addHandler($(document), mousemove, function (event) {
            ...
            if (that.isMouseDown && that.mouseDown && that.isTouchDevice() && !that.dragOrResize) {
                if (Math.abs(that.mouseDown.top - y) >= 10)
                {
                    if (new Date() - that.isMouseDownDate < 1000 && new Date() - that.isMouseDownDate > 100) {
                        var value = that.vScrollInstance.value;
                        if (that.vScrollBar[0].style.visibility != "hidden") {
                            that.vScrollInstance.setPosition(value - y + that.mouseDown.top);
                            event.stopPropagation();
                            event.preventDefault();
                ...
                if (Math.abs(that.mouseDown.left - x) >= 10){
                    if (new Date() - that.isMouseDownDate < 1000 && new Date() - that.isMouseDownDate > 100) {
                        var value = that.hScrollInstance.value;
                        if (that.hScrollBar[0].style.visibility != "hidden") {
                            that.hScrollInstance.setPosition(value - x + that.mouseDown.left);
                            event.stopPropagation();
                            event.preventDefault();
                            that.isMouseDownDate = new Date();
                        }
        ...
        var clickHandler = function (event) {
            ...
            if (appointment && appointment[0].className.indexOf('feedback') == -1) {
                ...
                if (selection !== null) {
                    ...
                    openMenu(false);
                    that._raiseEvent('cellClick', { cell: td });
                }
            }
            else if (event.which != 3) {
                that.clearAppointmentsSelection();
            ...
            if (key !== undefined && event.which != 3) {
                ...
                if (rowdata) {
                    ...
                    if (that._lastSelectedCell && that._lastSelectedCell.getAttribute('data-selected') == "true" && td.getAttribute('data-selected') === "true" && (time - that.clickTime.getTime() < timeInterval)) {
                        // double click.
                        that._raiseEvent('cellDoubleClick', { cell: td });
                        doubleClick = true;
                        that._initDialog();
                        that._openDialog();
                        if (event.preventDefault) {
    ...
    _initializeColumns: function () {
        ...
        var createtimelineDayColumns = function (date, resourceName) {
            ...
            for (var i = 0; i < columnsCount; i++) {
                ...
                that.columns.push({ columnGroup: resourceName, text: cellvalue, width: width });
            }
        }
            ...
            for (var r = 0; r < date.appointments.length; r++) {
                ...
                for (var j = 0; j < columnslength; j++) {
                    ...
                    if (columnIndex == 1) {
                        ...
                        if (appointment.resourceId) {
                            $.each(that._resources, function (index, value) {
                                if (appointment.resourceId == value) {
                                    ...
                                    var element = "<div class='" + className + "' style='margin-right: 5px; border-color: " + colors.border + "; background: " + colors.background + ";' class='" + that.toThemeProperty('jqx-scheduler-legend') + "'></div>";
                                    cellvalue = element;
                                }
                            });
                        }
                        ...
                        cellvalue += "<span title='" + tooltip + "' class='" + className + "'>" + title + "</span>";
                    }
