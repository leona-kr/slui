    ...
    $.extend($.jqx._jqxDataTable.prototype, {
        defineInstance: function () {
            var settings = {
                ...
                scrollBarSize: 10,//$.jqx.utilities.scrollBarSize,
        ...
        _builddataloadelement: function () {
            ...
            var table = $("<div />");//$('<div style="z-index: 99999; margin-left: -66px; left: 50%; top: 50%; margin-top: -24px; position: relative; width: 100px; height: 33px; padding: 5px; font-family: verdana; font-size: 12px; color: #767676; border-color: #898989; border-width: 1px; border-style: solid; background: #f6f6f6; border-collapse: collapse;"><div style="float: left;"><div style="float: left; overflow: hidden; width: 32px; height: 32px;" class="jqx-grid-load"/><span style="margin-top: 10px; float: left; display: block; margin-left: 5px;" >' + this.gridlocalization.loadtext + '</span></div></div>');
        ...
        localizestrings: function (localizationobj, refresh) {
            ...
            if (localizationobj != null) {
                ...
                this.gridlocalization =
                {
                    ...
                    decimalseparator: '.',
                    thousandsseparator: ',',
                    pagergotopagestring: "Page:",		//Go to page:
                    pagershowrowsstring: "Per Page",		//Show rows:
                    pagerrangestring: " of ",
        ...
        _initpager: function () {
            ...
            if (!this.pagerRenderer) {
                ...
                var createPagerElements = function (pagerdiv, name) {
					...
					var totalpage = Math.ceil(that.dataview.totalrecords/that.dataview.pagesize);
                    var input = $('<div style="margin-right: 7px;height: 17px; float: right;"><input style="margin-top: 0px; text-align: right; width: 20px;" type="text"/> /<span style="margin-right:7px;">'+ totalpage +'</span></div>');
                    ...
                    var pagershowrowscombo = $('<div id="gridpagerlist" style="margin-top: 0px; float: right;"></div>');
					var download = {};

					if(that.exportArea.length !=0){
						var icons = that.exportArea;
						for(var i=0, len =icons.length; i<len ;i++){
							...
							if( icons[i].confirm == true){
								var msg = icons[i].confirmMsg ? icons[i].confirmMsg : that.confirmMsg;
								$button.data({"confirm": true, "confirmMsg" : msg});
							}

							$button.html('<span class="icon icon-'+icons[i].icon+'"></span>'+icons[i].text);

							download[i] = $button;
							details.append(download[i]);
						}
					}

					details.find("button").each(function(){
						var type = $(this).data("file-type");
						if( type == "other"){
							$(this).on("click",$(this).data("onClick"));
						} else {
							$(this).on("click",function(){
								if($(this).data("confirm") && $(this).data("confirmMsg")){
									if(!confirm($(this).data("confirmMsg"))) return false;
								}
								that.exportData(type)
							});
						}
					});
					...
					*/
                    ...
                    if (!that.rtl) {
                        ...
                        pagerdiv.append('<div style="float:left;margin-left:7px;">Total <b>'+that.dataview.totalrecords+'</b> rows</div>');
                    }
                    if (that.pagerMode != "default") {
                        if (!that.rtl) {
						...
						pagershowrowscombo.jqxDropDownList({
                        ...
                        this.addHandler(pagershowrowscombo, 'select', function (event) {
                            if (event.args) {
                                ...
                                if (pageSize >= 100) {
                                    that["pagershowrowscombotop"].jqxDropDownList({ width: that.widthPageSelect+7 });//that["pagershowrowscombotop"].jqxDropDownList({ width: 55 });
                                    that["pagershowrowscombobottom"].jqxDropDownList({ width: that.widthPageSelect+7});//that["pagershowrowscombobottom"].jqxDropDownList({ width: 55 });
                                }
                                else {
                                    that["pagershowrowscombotop"].jqxDropDownList({ width: that.widthPageSelect });//that["pagershowrowscombotop"].jqxDropDownList({ width: 44 });
                                    that["pagershowrowscombobottom"].jqxDropDownList({ width: that.widthPageSelect });//that["pagershowrowscombobottom"].jqxDropDownList({ width: 44 });
                                }
        ...
        updatepagerdetails: function () {
            ...
            if (this.pagerMode != "default") {
                if (this["pagergotoinputbottom"]) {
                    ...
                    input.val(this.dataview.pagenum + 1);
                    input.attr('title', '1 - ' + pagescount);
					input.siblings("span").text( Math.ceil(that.dataview.totalrecords/that.dataview.pagesize) );
                }
            }
            else {
                var anchors = "";
            ...
            /*if (this["pagerdetailsbottom"]) {
                if (!this.rtl) {
					...
					//this["pagerdetailstop"][0].innerHTML = "/" + totalrecords;
                }
                else {
